...
def should_include(entry):
    """Determine if an entry should appear in the dictionary output."""
    entry_type = entry[COL_ENTRY_TYPE]
    if entry_type in INCLUDE_TYPES:
        return True
...
def main():
    ...
    output_file = sys.argv[2] if len(sys.argv) > 2 else 'dict-twblg.json'
